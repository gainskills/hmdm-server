...
public interface ApplicationMapper {
    ...
    String SELECT_BASE =
            "SELECT applications.*, customers.name AS customerName, customers.master AS commonApplication, "
                    ...
                    + "FROM applications " + "INNER JOIN customers ON customers.id = applications.customerId "
    ...
    String SELECT_VERSION_BASE = "SELECT " + "applicationVersions.id, " + "applicationVersions.applicationId, "
            + "applicationVersions.version, " + "applicationVersions.versionCode, "
            + "applicationVersions.url, "
            + "applicationVersions.split, " + "applicationVersions.urlArmeabi, "
            ...
            + "customers.master AS commonApplication," + "applications.system, " + "applications.type "
            + "FROM applicationVersions "
    ...
    // " FROM configurations " +
    // " WHERE configurations.customerId=#{customerId})"})
    // void removeApplicationConfigurationsById(@Param("customerId") int customerId, @Param("id") Integer
    // applicationId);
    ...
    void insertApplicationConfigurations(
    ...
    @Select({SELECT_BY_VERSION_BASE + "WHERE pkg = #{pkg}"})
    List<Application> findAllByPackageId(@Param("pkg") String pkg);

    @Select({SELECT_BASE + "WHERE applications.id = #{id}"})
    Application findById(@Param("id") int id);

    @Select({SELECT_BASE + "ORDER BY name"})
    ...
    void changeConfigurationsApplication(
            @Param("appId") Integer oldAppId,
    ...
    @Select(SELECT_VERSION_BASE + "WHERE applicationVersions.applicationId=#{id} "
    ...
    @Select(SELECT_VERSION_BASE + "WHERE (customerId = #{customerId} OR customers.master = TRUE) "
    ...
    ApplicationVersion findApplicationVersionByCode(
            @Param("customerId") int customerId, @Param("pkg") String pkg, @Param("versionCode") int versionCode);
    ...
    @Update("UPDATE applications " + "SET latestVersion = (" + "            SELECT id "
            ...
            + "                 WHERE apv2.applicationId = applications.id) " + "            LIMIT 1) "
            + "WHERE id = #{id}")
    void recalculateLatestVersion(@Param("id") Integer applicationId);

    @Update("UPDATE configurationApplications " + "SET applicationVersionId = #{newId} "
            + "WHERE applicationId = #{appId} " + "AND action <> 2 " + "AND EXISTS (SELECT 1 "
            + "            FROM configurations "
            + "            WHERE configurations.id = configurationApplications.configurationId "
            + "            AND configurations.autoUpdate IS TRUE)")
    int autoUpdateConfigurationsApplication(
            @Param("appId") Integer applicationId, @Param("newId") Integer newAppVersionId);

    @Update("UPDATE configurations " + "SET mainAppId = #{newId} " + "WHERE configurations.autoUpdate IS TRUE "
    ...
    int autoUpdateConfigurationsMainApplication(
            @Param("appId") Integer applicationId, @Param("newId") Integer newAppVersionId);

    // TODO : ISV : Re-check methods must be updated to find the application by package ID but not applicationId
    @Update("UPDATE configurations " + "SET mainAppId = (" + "                 SELECT ca.applicationVersionId "
    ...
    @Update("UPDATE configurations " + "SET mainAppId = (" + "                 SELECT ca.applicationVersionId "
            ...
            + "                                 INNER JOIN applications apps2 ON apps2.id = av.applicationId "
            + "                                 WHERE av.id = configurations.mainAppId)" + ") "
            + "WHERE configurations.id = #{configurationId} " + "AND NOT configurations.mainAppId IS NULL")
    int recheckConfigurationMainApplication(@Param("configurationId") Integer configurationId);

    @Update("UPDATE configurations " + "SET contentAppId = (" + "                 SELECT ca.applicationVersionId "
            + "                 FROM configurationApplications ca "
            + "                 INNER JOIN applications apps ON apps.id = ca.applicationId "
            + "                 WHERE ca.configurationId = configurations.id "
            + "                 AND ca.action = 1 "
    ...
    @Update("UPDATE configurations " + "SET kioskMode = false "
    ...
    @Update("UPDATE configurations " + "SET contentAppId = #{newId} " + "WHERE configurations.autoUpdate IS TRUE "
            + "AND EXISTS (SELECT 1 FROM applicationVersions "
            + "            WHERE applicationVersions.id = configurations.contentAppId"
            + "            AND applicationVersions.applicationId = #{appId})")
    int autoUpdateConfigurationsContentApplication(
            @Param("appId") Integer applicationId, @Param("newId") Integer newAppVersionId);

    @Select("SELECT id " + "FROM applicationVersions apv1 " + "WHERE apv1.applicationId = #{id} "
            + "AND (SELECT COUNT(*) " + "     FROM applicationVersions apv2 "
    ...
    @Delete("DELETE FROM configurationApplications " + "WHERE configurationId = #{configurationId} "
            + "AND action <> 2 " + "AND applicationId = (SELECT applicationVersions.applicationId "
            + "                     FROM applicationVersions "
            + "                     WHERE applicationVersions.id = #{versionId})")
    int uninstallOtherVersions(
            @Param("versionId") int applicationVersionId, @Param("configurationId") int configurationId);
    ...
    List<LookupItem> resolveAppsByPackageId(
            @Param("customerId") Integer customerId, @Param("apps") Collection<String> appPackages);

    @Select("SELECT applications.id AS id, " + "       applications.pkg AS name " + "FROM applications "
            + "INNER JOIN customers ON customers.id = applications.customerid "
            + "WHERE (applications.customerId = #{customerId} OR customers.master IS TRUE) "
            + "AND applications.pkg ILIKE #{filter} " + "LIMIT #{pageSize}")
    List<LookupItem> findMatchingApplicationPackages(
            @Param("customerId") Integer customerId, @Param("filter") String filter, @Param("pageSize") int pageSize);
    ...
    @Select("SELECT COALESCE( " + "(SELECT id FROM applicationVersions " + "WHERE applicationId = #{appId} "
            + "AND version = #{versionNumber} AND id <> #{appVersionId}), 0)")
    int getDuplicateVersionForApp(
            @Param("appId") Integer applicationId,
    ...
    @Delete("DELETE FROM configurationApplications " + "WHERE configurationId = #{configurationId} "
            ...
            + "            WHERE pkg = ("
            + "                SELECT pkg " + "                FROM applications "
            + "                WHERE id=#{applicationId} " + "            ) AND id != #{applicationId} "
            + ")")
    int deleteApplicationConfigurationLinksForSamePkg(
