...
public interface ConfigurationFileMapper {
    ...
    @Select("SELECT cf.id AS id, " + "       cf.configurationId AS configurationId,  "
            ...
            + "       f.uploadTime AS lastUpdate,  " + "       cf.fileId AS fileId,  " + "       cf.remove AS remove,  "
            + "       f.replaceVariables AS replaceVariables  " + "FROM configurationFiles cf "
    ...
    @Select("SELECT COUNT(*) AS cnt " + "FROM configurationFiles cf " + "WHERE cf.fileId = #{fileId}")
    ...
    @Select("SELECT c.name " + "FROM configurationFiles cf "
            + "INNER JOIN configurations c ON c.id = cf.configurationId " + "WHERE cf.fileId = #{fileId}")
    List<String> getUsingConfigurations(
    ...
    @Select({"SELECT configurationFiles.id             AS id, "
            ...
            + "       configurationFiles.id IS NOT NULL  AS upload " + "FROM configurations "
    ...
    List<FileConfigurationLink> getConfigurationFileLinks(
