...
public interface DeviceInfoMapper {
    ...
    @Select("SELECT settings.* " + "FROM plugin_deviceinfo_settings settings " + "WHERE customerId = #{customerId}")
    DeviceInfoPluginSettings findPluginSettingsByCustomerId(@Param("customerId") Integer customerId);

    @Insert({
                    "INSERT INTO plugin_deviceinfo_settings " + "(" + "dataPreservePeriod, intervalMins, sendData, customerId"
                            + ") " + "VALUES (" + "#{dataPreservePeriod}, #{intervalMins}, #{sendData}, #{customerId}" + ") "
                            + "ON CONFLICT ON CONSTRAINT plugin_deviceinfo_settings_customer_unique DO " + "UPDATE SET "
                            + "dataPreservePeriod = EXCLUDED.dataPreservePeriod, " + "intervalMins = EXCLUDED.intervalMins, "
    ...
    @SelectKey(
            statement = "SELECT currval('plugin_deviceinfo_deviceParams_id_seq')",
            keyColumn = "id",
            keyProperty = "id",
            before = false,
    ...
    @Insert("INSERT INTO plugin_deviceinfo_deviceParams_device (" + "  recordId," + "  batteryLevel,"
            + "  batteryCharging," + "  ip," + "  keyguard," + "  ringVolume," + "  wifi," + "  mobileData," + "  gps,"
            + "  bluetooth," + "  usbStorage," + "  memoryTotal," + "  memoryAvailable" + ") VALUES ("
            + "  #{recordId}," + "  #{item.batteryLevel}," + "  #{item.batteryCharging}," + "  #{item.ip},"
            + "  #{item.keyguard}," + "  #{item.ringVolume}," + "  #{item.wifi}," + "  #{item.mobileData},"
            + "  #{item.gps}," + "  #{item.bluetooth}," + "  #{item.usbStorage}," + "  #{item.memoryTotal},"
            + "  #{item.memoryAvailable}" + ")")
    int insertDeviceInfoGroupDevice(@Param("recordId") Integer recordId, @Param("item") DeviceData device);

    @Insert("INSERT INTO plugin_deviceinfo_deviceParams_wifi (" + "  recordId," + "  rssi," + "  ssid," + "  security,"
            + "  state," + "  ip," + "  tx," + "  rx" + ") VALUES (" + "  #{recordId}," + "  #{item.rssi},"
            + "  #{item.ssid}," + "  #{item.security}," + "  #{item.state}," + "  #{item.ip}," + "  #{item.tx},"
            + "  #{item.rx}" + ")")
    int insertDeviceInfoGroupWifi(@Param("recordId") Integer recordId, @Param("item") WifiData wifi);

    @Insert("INSERT INTO plugin_deviceinfo_deviceParams_gps (" + "  recordId," + "  state," + "  lat," + "  lon,"
            + "  alt," + "  speed," + "  course" + ") VALUES (" + "  #{recordId}," + "  #{item.state},"
            + "  #{item.lat}," + "  #{item.lon}," + "  #{item.alt}," + "  #{item.speed}," + "  #{item.course}" + ")")
    int insertDeviceInfoGroupGps(@Param("recordId") Integer recordId, @Param("item") GpsData gps);

    @Insert("INSERT INTO plugin_deviceinfo_deviceParams_mobile (" + "  recordId," + "  rssi," + "  carrier," + "  data,"
            + "  ip," + "  state," + "  simState," + "  tx," + "  rx" + ") VALUES (" + "  #{recordId},"
            + "  #{item.rssi}," + "  #{item.carrier}," + "  #{item.data}," + "  #{item.ip}," + "  #{item.state},"
            + "  #{item.simState}," + "  #{item.tx}," + "  #{item.rx}" + ")")
    int insertDeviceInfoGroupMobile(@Param("recordId") Integer recordId, @Param("item") MobileData mobile);

    @Insert("INSERT INTO plugin_deviceinfo_deviceParams_mobile2 (" + "  recordId," + "  rssi," + "  carrier,"
            + "  data," + "  ip," + "  state," + "  simState," + "  tx," + "  rx" + ") VALUES (" + "  #{recordId},"
            + "  #{item.rssi}," + "  #{item.carrier}," + "  #{item.data}," + "  #{item.ip}," + "  #{item.state},"
            + "  #{item.simState}," + "  #{item.tx}," + "  #{item.rx}" + ")")
    ...
    @Select("SELECT " + "    devices.id AS id," + "    devices.number AS deviceNumber,"
            + "    devices.description AS description," + "    devices.lastUpdate AS latestUpdateTime, "
            + "    devices.imei AS imeiRequired, " + "    devices.infojson ->> 'imei' AS imeiActual, "
            + "    devices.phone AS phoneNumberRequired, " + "    devices.infojson ->> 'phone' AS phoneNumberActual, "
            + "    devices.infojson ->> 'model' AS model, " + "    devices.infojson ->> 'androidVersion' AS osVersion, "
            + "    devices.infojson ->> 'batteryLevel' AS batteryLevel,"
            + "    devices.infojson ->> 'mdmMode' AS mdmMode," + "    devices.infojson ->> 'kioskMode' AS kioskMode,"
            ...
            + "    devices.infojson ->> 'phone2' AS phone2," + "    devices.infojson ->> 'imei2' AS imei2,"
            + "    devices.infojson ->> 'iccid' AS iccid," + "    devices.infojson ->> 'imsi' AS imsi,"
            + "    devices.infojson ->> 'iccid2' AS iccid2," + "    devices.infojson ->> 'imsi2' AS imsi2,"
            + "    devices.infojson ->> 'serial' AS serial," + "    devices.infojson ->> 'cpu' AS cpu,"
            ...
            + "FROM devices " + "WHERE devices.id = #{id}")
    DeviceInfo getDetailedDeviceInfo(@Param("id") int deviceId);

    @Select("SELECT " + "    groups.id AS id," + "    groups.name AS name " + "FROM deviceGroups "
            + "INNER JOIN groups ON groups.id = deviceGroups.groupId " + "WHERE deviceGroups.deviceId = #{id} "
            + "ORDER BY groups.name ")
    List<LookupItem> getDeviceGroups(@Param("id") int deviceId);

    @Select("SELECT " + "    p.ts AS latestUpdateTime," + "" + "    main.batteryLevel AS deviceBatteryLevel,"
            + "    main.batteryCharging AS deviceBatteryCharging," + "    main.ip AS deviceIpAddress,"
            + "    main.keyguard AS deviceKeyguard," + "    main.ringVolume AS deviceRingVolume,"
            + "    main.wifi AS deviceWifiEnabled," + "    main.mobileData AS deviceMobileDataEnabled,"
            + "    main.gps AS deviceGpsEnabled," + "    main.bluetooth AS deviceBluetoothEnabled,"
            + "    main.usbStorage AS deviceUsbEnabled," + "    main.memoryTotal AS deviceMemoryTotal,"
            + "    main.memoryAvailable AS deviceMemoryAvailable," + "" + "    wifi.rssi AS wifiRssi,"
            + "    wifi.ssid AS wifiSsid," + "    wifi.security AS wifiSecurity," + "    wifi.state AS wifiState,"
            + "    wifi.ip AS wifiIpAddress," + "    wifi.tx AS wifiTx," + "    wifi.rx AS wifiRx," + ""
            + "    gps.state AS gpsState," + "    gps.lat AS gpsLat," + "    gps.lon AS gpsLon,"
            + "    gps.alt AS gpsAlt," + "    gps.speed AS gpsSpeed," + "    gps.course AS gpsCourse," + ""
            + "    mobile.rssi AS mobile1Rssi," + "    mobile.carrier AS mobile1Carrier,"
            + "    mobile.data AS mobile1DataEnabled," + "    mobile.ip AS mobile1IpAddress,"
            + "    mobile.state AS mobile1State," + "    mobile.simState AS mobile1SimState,"
            + "    mobile.tx AS mobile1Tx," + "    mobile.rx AS mobile1Rx," + "" + "    mobile2.rssi AS mobile2Rssi,"
            + "    mobile2.carrier AS mobile2Carrier," + "    mobile2.data AS mobile2DataEnabled,"
            + "    mobile2.ip AS mobile2IpAddress," + "    mobile2.state AS mobile2State,"
            + "    mobile2.simState AS mobile2SimState," + "    mobile2.tx AS mobile2Tx,"
            + "    mobile2.rx AS mobile2Rx" + " " + "FROM plugin_deviceinfo_deviceParams p "
